...
- **The Instruction Limit**: SwapVM's `runLoop` uses a `uint8` instruction length (`uint256 argsLength = uint8(programBytes[pc++]);`), which caps calldata args to 255 bytes per instruction. With 6 bytes overhead (`siblingCount: 2`, `maxUtilBps: 2`, `widenBps: 2`), exactly 7 sibling hashes (`32 * 7 = 224`) fit in a single opcode payload. This is explicitly enforced on-chain via `EncumbranceArgsBuilder.MAX_INSTRUCTION_SIBLINGS = 7`.
- **The Real-World Gap**: On Ethereum mainnet, our index proved makers holding up to 257 strategies on a single token (`COUNCIL-VERDICT.md` §1). If a maker with >7 live strategies passes only 7, the remaining strategies are not inspected by this instruction.
- **Current Status**: Refusal of strategies with >7 live siblings by `Tap.sol` and the subgraph index is **explicitly future work, not done in this pass**. The current instruction strictly enforces solvency against the supplied sibling set (up to 7). Do not claim the system-wide gate is finished until `Tap.sol` and the subgraph check are implemented and tested.
